...
function Sparkline({ a = [], b = [] as number[] }: { a?: number[]; b?: number[] }) {
  const W = 160, H = 40, P = 4, all = [...a, ...b], mn = Math.min(...all, 0.99), mx = Math.max(...all, 1.01);
...
function CostBreakdownBar({ segments }: { segments: { key: string; label: string; amount: number }[] }) {
  const total = Math.max(1, segments.reduce((s, x) => s + x.amount, 0));
  const palette = ["bg-blue-600", "bg-indigo-400", "bg-emerald-500", "bg-amber-500", "bg-rose-500", "bg-gray-400"];
  ...
  return (
    <div>
      ...
      <div className="mt-2 grid sm:grid-cols-3 gap-2">
        {segments.map((s, i) => (
          <div key={s.key} className="flex items-center gap-2 text-xs text-gray-700">
            ...
            <span className="text-gray-400">({Math.round((s.amount / total) * 100)}%)</span>
          </div>
        ))}
      </div>
    </div>
...
export default function Dashboard() {
  ...
  const fee = 3950, ops = 850;
  const includes = { water: true, heat: true, broadband: false, parking: false } as const;

  const breakdown = React.useMemo(() => {
    const base = fee, water = base * 0.05, heat = base * 0.12, broadband = base * 0.06, parking = includes.parking ? base * 0.1 : 0, other = Math.max(0, base - (water + heat + broadband + parking));
    return [
  ...
  const seriesA = propertyData.priceHistory.map((p) => p.price / propertyData.priceHistory[0].price);
  const seriesB = seriesA.map((v, i) => v * (i < seriesA.length / 2 ? 0.98 : 0.99));
    ...
    <div className="min-h-screen bg-gradient-to-b from-white via-emerald-50/40 to-white">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        ...
        <div className="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
          <PriceChart
            data={propertyData.priceHistory.map(({ date, price }) => ({ month: date, price }))}
        ...
        <section className="mt-8 grid gap-6 xl:grid-cols-3">
          ...
          <div className="rounded-2xl p-4 bg-white shadow-sm border border-gray-100">
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <Badge text={`OVK ${ovkApproved ? messages[lang].fields.approved : messages[lang].fields.notApproved}`} tone={ovkApproved ? "green" : "red"} />
              <Badge text={`${messages[lang].fields.declared} ${energy.declared}`} tone="blue" />
            </div>
          ...
          {/* Costs */}
          <div className="rounded-2xl p-4 bg-white shadow-sm border border-gray-100">
            <PanelHeader title={messages[lang].panels.costs} icon={<CostIcon />} />
            <div className="grid grid-cols-3 gap-2 mb-2">
              <div className="rounded-xl border p-2 text-center">
                <div className="text-[10px] text-gray-500">{messages[lang].fields.fee}</div>
                <div className="text-sm font-semibold">{fee.toLocaleString("sv-SE")} kr</div>
              </div>
              <div className="rounded-xl border p-2 text-center">
                <div className="text-[10px] text-gray-500">{messages[lang].fields.ops}</div>
                <div className="text-sm font-semibold">{ops.toLocaleString("sv-SE")} kr</div>
              </div>
              <div className="rounded-xl border p-2 text-center">
                <div className="text-[10px] text-gray-500">Parking</div>
                <div className="text-sm font-semibold">{includes.parking ? (lang === "sv" ? "Ja" : "Yes") : lang === "sv" ? "Nej" : "No"}</div>
              </div>
            </div>
            {/* Fee + ops stacked bar */}
            <div className="mt-1 h-2 w-full bg-gray-100 rounded-full overflow-hidden">
              <div className="h-2 flex">
                <div className="bg-blue-500" style={{ width: `${(fee / (fee + ops)) * 100}%` }} />
                <div className="bg-indigo-300" style={{ width: `${(ops / (fee + ops)) * 100}%` }} />
              </div>
            </div>
            <div className="mt-2 text-xs text-gray-500">{messages[lang].fields.feeVsOps}</div>
            <div className="mt-4">
              <div className="text-xs text-gray-500 mb-1">{messages[lang].fields.detailedBreakdown}</div>
          ...
          {/* Market */}
          <div className="rounded-2xl p-4 bg-white shadow-sm border border-gray-100">
            <PanelHeader title={messages[lang].panels.market} icon={<MarketIcon />} />
            <InfoRow
              left={messages[lang].panels.areaAvg}
              right={`~${(propertyData.valueInterval.min / 1_000_000).toFixed(1)}–${(propertyData.valueInterval.max / 1_000_000).toFixed(1)} M SEK`}
            />
            <InfoRow left={messages[lang].panels.trend} right={lang === "sv" ? "+2,1%/år" : "+2.1%/yr"} />
            <div className="mt-2"><Sparkline a={seriesA} b={seriesB} /></div>
            <div className="my-2 border-t" />
            <div className="flex gap-2 flex-wrap text-xs">
              <Badge text={`${messages[lang].panels.transit}: 200 m`} tone="blue" />
              <Badge text={`${messages[lang].panels.noise}: ${lang === "sv" ? "Låg" : "Low"}`} tone="amber" />
              <Badge text={`${messages[lang].panels.center}: 1,2 km`} tone="gray" />
            </div>
          </div>
        </section>
...
function SimpleLogo() {
  return (
    <svg viewBox="0 0 64 64" className="h-12 w-12" aria-hidden="true">
      ...
      <path d="M8 22 L32 8 L56 22" fill="none" stroke="#111827" strokeWidth="3" />
    </svg>
  );
}
...
function DevTests() {
  ...
  React.useEffect(() => {
    ...
    try {
      const sample = { healthScore: 78, valueInterval: { min: 4500000, max: 5500000, confidence: 85 }, riskLevel: "medium" } as any;
    ...
    try {
      ...
      pass++;
    } catch (e: any) { fail++; logs.push("seriesA baseline test failed: " + e?.message); }

    // 7) Extra: priceHistory sanity
    try {
      const ps = propertyData.priceHistory.map((p) => p.price);
      console.assert(ps.length > 1 && ps.every((v) => Number.isFinite(v)), "priceHistory should contain finite numbers");
      pass++;
    } catch (e: any) { fail++; logs.push("priceHistory sanity test failed: " + e?.message); }

    // 8) Extra: risk keys exist
    try {
      const keys = Object.keys(messages.en.summaryTail);
      console.assert(["low", "medium", "high"].every((k) => keys.includes(k)), "summaryTail missing risk keys");
      pass++;
    } catch (e: any) { fail++; logs.push("summaryTail keys test failed: " + e?.message); }

    // 9) Extra: fee/ops stacked bar sums ~100
    try {
      const fee = 3950, ops = 850;
      const pct = (fee / (fee + ops)) * 100 + (ops / (fee + ops)) * 100;
      console.assert(Math.abs(pct - 100) < 0.0001, "Fee+Ops widths must sum to 100%");
      pass++;
    } catch (e: any) { fail++; logs.push("Fee/Ops width test failed: " + e?.message); }
